...
- durable reservation and API-key storage with memory, SQLite, or Postgres
  options
- personal, everyone, and team-shared reservation profiles with nested-team access
...
| `CONTROL_PLANE_MAINTENANCE_MODE` | `false` | Initial/default maintenance choice when no administrator override has been stored |
| `CONTROL_PLANE_FORCE_MAINTENANCE_MODE` | `false` | Force maintenance for protected storage work; cannot be disabled in the application |
| `CONTROL_PLANE_MAINTENANCE_STATE_PATH` | next to storage lock | Durable administrator maintenance choice |
