...
export class IdentityService {
  ...
  async renameUser(actor: AuthenticatedUser, userId: string, input: Pick<UserAccount, "username" | "displayName">) { this.requirePermission(actor,"users.manage"); if(await this.userHasWildcard(userId))this.requireOwner(actor,"Only an Owner can rename an Owner account"); return this.repository.renameUser(userId,input,new Date(),actor.id); }
