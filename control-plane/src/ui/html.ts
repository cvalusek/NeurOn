...
export function userAdminPage(
  ...
  const userRows = users.map((account) => `<details class="drilldown"><summary><div><strong>${escapeHtml(account.displayName ?? account.username)}</strong><div class="muted"><code>${escapeHtml(account.username)}</code> · ${escapeHtml(account.id)}</div></div><span class="badge ${account.status === "active" ? "active" : "failed"}">${escapeHtml(account.status)}</span></summary><div class="drilldown-body">
    ...
    ${account.status === "active" && !account.mergedIntoUserId ? `<form method="post" action="/admin/users/${escapeHtml(account.id)}/name" onsubmit="return confirm('Rename this account and revoke its active sessions?')"><div class="field-grid"><p><label>Canonical username<br><input name="username" value="${escapeHtml(account.username)}" maxlength="200" required></label></p><p><label>Display name<br><input name="displayName" value="${escapeHtml(account.displayName ?? "")}" maxlength="200"></label></p></div><p class="muted">Provider subjects stay linked. If this name belongs to a disabled alias merged into this account, NeurOn archives that alias automatically.</p><button class="secondary" type="submit">Save account name</button></form>` : ""}
